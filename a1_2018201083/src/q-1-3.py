...
def info_miss(positive,negative):
	total =0
	feature=[]
	...
	for i in feature:
		...
		if(n==0):
			continue
		infor = infor + (n/total)*miss(i[0],i[1])
	return infor
...
def feature_select_gini(data,features,value_dict):
	...
	for f in features:
		...
		# print("negative list= ",negative_list)
			...
			information = info_gini(positive_list,negative_list)
...
def feature_select_miss(data,features,value_dict):
	global label
	# cols=data.columns
	max_gain=-1
	# print("features in fselect= ",features)
	...
	for f in features:
		...
		# print("negative list= ",negative_list)
			...
			information = info_miss(positive_list,negative_list)
			gain = info_gain_miss(miss_f,information)
		# print("gain= ",gain)
		# print("feature= ",f)
			# print("gain= ",gain)
			if(gain<0):
...
def make_tree_miss(data,features,value_dict,n):
	# if(len(features)==0):
	# 	print("len of feature ==0")
	# 	return
	# print("features= ",features)
	...
	f= feature_select_miss(data,features,value_dict)
	print("feature selected= ",f)
	# f= [f_selected,split,pos_selected,neg_selected]
	...
	data_tosend= data.loc[data[f[0]]<=value]
	# print("data2= \n",data_tosend)
	make_tree_miss(data_tosend,temp,ldict,n1)
	
	#RIGHT CHILD
	n2=node()
	n.children["high"]=n2
	data_tosend2= data.loc[data[f[0]]>value]
	make_tree_miss(data_tosend2,temp,rdict,n2)
...
data_train_miss = copy.deepcopy(data_train)
features_miss = copy.deepcopy(features)
value_dict_miss =  copy.deepcopy(value_dict)

root= node()
make_tree(data_train,features,value_dict,root)


print("gini trainnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn")
root_gini = node()
make_tree_gini(data_train_gini,features_gini,value_dict_gini,root_gini)


print("miss trainnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn")
root_miss = node()
make_tree_miss(data_train_miss,features_miss,value_dict_miss,root_miss)
...
ypredict=predict(xTest,root_gini)
print("accuracy= ",accuracy(yTest,ypredict))
...
ypredict=predict(xTest,root_miss)
print("accuracy= ",accuracy(yTest,ypredict))
