...
def gini(positive,negative):
    ...
    prob_yes = prob(positive,total)
    prob_no = prob(negative,total)
    gini = 2*prob_yes*prob_no
    if(gini<0):
    	gini=0

    return gini

def info_gini(positive,negative):
	total =0
	feature=[]
	...
	for i in feature:
		...
		if(n==0):
			continue
		infor = infor + (n/total)*gini(i[0],i[1])
	return infor

def info_gain_gini(gini,info):
	gain = gini - info
	return gain
def feature_select_gini(data,features,value_dict):
	global label
	# cols=data.columns
	max_gain=-1
	...
	for f in features:
		...
		# print("negative list= ",negative_list)
			...
			information = info_ginni(positive_list,negative_list)
			gain = info_gain_gini(gini_f,information)
		# print("gain= ",gain)
		# print("feature= ",f)
			# print("gain= ",gain)
			if(gain<0):
...
def make_tree_gini(data,features,value_dict,n):
	# if(len(features)==0):
	...
	f= feature_select_gini(data,features,value_dict)
	...
	# print("data2= \n",data_tosend)
	make_tree_gini(data_tosend,temp,ldict,n1)
	
	#RIGHT CHILD
	n2=node()
	n.children["high"]=n2
	data_tosend2= data.loc[data[f[0]]>value]
	make_tree_gini(data_tosend2,temp,rdict,n2)




def misclass(positive,negative):
    total = positive +negative
    prob_yes = prob(positive,total)
    prob_no = prob(negative,total)
    miss= min(prob_yes,prob_no)

    return miss

def info_missclass(positive,negative):
	total =0
	feature=[]
	...
	for i in feature:
		...
		if(n==0):
			continue
		infor = infor + (n/total)*missclass(i[0],i[1])
	return infor

def info_gain_missclass(missclass,info):
	gain = missclass - info
	return gain
def feature_select_missclass(data,features,value_dict):
	global label
	# cols=data.columns
	max_gain=-1
	# print("features in fselect= ",features)
	...
	for f in features:
		...
		lpos_f= len(positive_f)
		lneg_f= len(negative_f)
		missclass_f= missclass(lpos_f,lneg_f)
		values= value_dict[f]
		# positive_list =[]
		# negative_list=[]
		for value in values:
		...
		# print("negative list= ",negative_list)
			...
			information = info_missclass(positive_list,negative_list)
			gain = info_gain_missclass(gini_f,information)
		# print("gain= ",gain)
		# print("feature= ",f)
			# print("gain= ",gain)
			if(gain<0):
...
def make_tree_missclass(data,features,value_dict,n):
	# if(len(features)==0):
	# 	print("len of feature ==0")
	# 	return
	# print("features= ",features)
	...
	f= feature_select_missclass(data,features,value_dict)
	print("feature selected= ",f)
	# f= [f_selected,split,pos_selected,neg_selected]
	...
	data_tosend= data.loc[data[f[0]]<=value]
	# print("data2= \n",data_tosend)
	make_tree_missclass(data_tosend,temp,ldict,n1)
	
	#RIGHT CHILD
	n2=node()
	n.children["high"]=n2
	data_tosend2= data.loc[data[f[0]]>value]
	make_tree_missclass(data_tosend2,temp,rdict,n2)
...
data_train_gini= copy.deepcopy(data_train)
features_gini = copy.deepcopy(features)
value_dict_gini=copy.deepcopy(value_dict)


root_gini= node()
make_tree(data_train_gini,features_gini,value_dict_gini,root_gini)
...
data_train_missclass= copy.deepcopy(data_train)
features_missclass = copy.deepcopy(features)
value_dict_missclass=copy.deepcopy(value_dict)


root_missclass= node()
make_tree(data_train_missclass,features_missclass,value_dict_missclass,root_missclass)
...
#########       GINI    ###############################
#######################################################
ypredict_gini=predict(xTest,root_gini)
# yTest= yTest.tolist()
print("accuracy gini= ",accuracy(yTest,ypredict_gini))

l= parameters(yTest,ypredict_gini)
print("recall gini= ",recall(l[0],l[2]))
print("precision gini= ",precision(l[0],l[1]))

print(precision_score(yTest,ypredict_gini))
...
ypredict_missclass=predict(xTest,root_missclass)
# yTest= yTest.tolist()
print("accuracy missclass= ",accuracy(yTest,ypredict_missclass))

l= parameters(yTest,ypredict_missclass)
print("recall missclass= ",recall(l[0],l[2]))
print("precision missclass= ",precision(l[0],l[1]))

print(precision_score(yTest,ypredict_missclass))
